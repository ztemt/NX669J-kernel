...
int cypress_start_read_chip_id(unsigned long *siliconId, unsigned char *siliconRev, unsigned long* blVer)
{
    unsigned long i;
    unsigned long inSize = 0;
    unsigned long outSize = 0;
    ...
    err = CyBtldr_CreateEnterBootLoaderCmd(inBuf, &inSize, &outSize);
    dev_err(&cypress_client->dev, "cypress_touchkey:buf: %p, inSize:%d, outSize: %d, MAX_COMMAND_SIZE: %d\n", inBuf, inSize, outSize, MAX_COMMAND_SIZE);

    if (CYRET_SUCCESS == err)
       err = CyBtldr_TransferData(inBuf, inSize, outBuf, outSize);
	else
		dev_err(&cypress_client->dev, "cypress_touchkey:cypress_start_read_chip_id ret is [%x]\n", err);

    if (CYRET_SUCCESS == err)
        err = CyBtldr_ParseEnterBootLoaderCmdResult(outBuf, outSize, siliconId, siliconRev, blVer, &status);
	else
		dev_err(&cypress_client->dev, "cypress_touchkey:CyBtldr_TransferData ret is [%x]\n", err);
...
int cypress_end_read_chip_id(struct i2c_client *client)
{
    const unsigned char RESET = 0x00;
    unsigned long inSize;
    unsigned long outSize;
...
int cypress_read_chip_id(struct cypress_info *info)
{
	int val;
	int ret = -1;
	...
	do{
        ...
        if(fw_ver < 0){
           pr_err("cypress_touchkey:%s: read fw version error(%d)\n",  __func__, fw_ver);
           msleep(100);
		} else {
           break;
        }
	...
	info->fw_ver = fw_ver;
	printk(KERN_ERR"cypress_touchkey: current fw version:[0x%x]\n", fw_ver);
	val = i2c_smbus_write_byte_data(client, CYPRESS4000_TOUCHKEY_ENTER_BOOTLOADER_MODE, CYPRESS4000_TOUCHKEY_ENTER_BOOTLOADER_COMMON_VALUE);
	if (val < 0)
	{
		dev_err(&client->dev, "cypress_touchkey:wirte command 0xaa error\n", val);
	//    	return val;
	}
	client->addr = CYPRESS_BOOTLOADER_MODE_I2C_ADDR;
	msleep(400);

	ret = cypress_start_read_chip_id(&siliconId, &siliconRev, &blVer);
	if (ret != CYRET_SUCCESS) {
		printk(KERN_ERR"cypress_touchkey: cpress_start_read_chip_id error\n");
		client->addr = CYPRESS_I2C_ADDR;
		return ret;
	}
	ret = cypress_end_read_chip_id(client);
	if (ret != CYRET_SUCCESS) {
		printk(KERN_ERR"cypress_touchkey: cypress_end_read_chip_id error\n");
		client->addr = CYPRESS_I2C_ADDR;
		return ret;
	}
	info->siliconId = siliconId;
	info->siliconRev = siliconRev;
	printk(KERN_ERR"cypress_touchkey: siliconId = 0x%x, siliconRev = 0x%x\n", siliconId, siliconRev);
	msleep(800);
	client->addr = CYPRESS_I2C_ADDR;
	printk(KERN_ERR"cypress_touchkey: ret = 0x%x, ret = 0x%x\n", ret, ret);
	return ret;
}
