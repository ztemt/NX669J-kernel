...
static DEVICE_ATTR(update_en, S_IWUSR | S_IWGRP, NULL, goodix_sysfs_update_en_store);
static DEVICE_ATTR(fwsize, S_IRUGO | S_IWUSR | S_IWGRP, goodix_sysfs_fwsize_show,
...
*/
	...
	for (i = 0; i < ARRAY_SIZE(goodix_fwu_attrs) && !ret; i++)
		ret = sysfs_create_file(&core_data->pdev->dev.kobj, goodix_fwu_attrs[i]);
	if (ret) {
		ts_err("failed create fwu sysfs files");
		while (--i >= 0)
			sysfs_remove_file(&core_data->pdev->dev.kobj, goodix_fwu_attrs[i]);
	...
	ret = sysfs_create_bin_file(&core_data->pdev->dev.kobj, &fw_ctrl->attr_fwimage);
	if (ret) {
...
/*
	sysfs_remove_bin_file(&core_data->pdev->dev.kobj, &fw_ctrl->attr_fwimage);

	for (i = 0; i < ARRAY_SIZE(goodix_fwu_attrs); i++)
		sysfs_remove_file(&core_data->pdev->dev.kobj,
				goodix_fwu_attrs[i]);
*/
