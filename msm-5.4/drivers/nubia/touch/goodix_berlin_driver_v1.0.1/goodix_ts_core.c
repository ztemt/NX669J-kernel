                ...
                struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ts_core_data->fp_switch = core_data->fp_switch;
	ts_err("->%s fp_switch = %d", core_data->goodix_suspend ?"error set":"success set", input);
	ts_err("->%s  ts_core_data->fp_switch = %d",  ts_core_data->fp_switch ?"error set":"success set", input);
	return count;
	}
...
static void synaptics_nubiaCtl_Re_Sus(struct goodix_ts_core *core_data);

static  ssize_t synaptics_nubia_fpswitch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static  ssize_t synaptics_nubia_fpswitch_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static DEVICE_ATTR(fpswitch, S_IRUGO | S_IWUSR | S_IWGRP,
		   synaptics_nubia_fpswitch_show, synaptics_nubia_fpswitch_store);

static  ssize_t synaptics_nubia_fp_mode_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",ts_core_data->en_fpmode);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	ts_core_data->en_fpmode = en ;
	ts_info("fp_mode:%d", en);
	switch(en) {
		case FP_GESTURE_OFF:
			ts_core_data->tp_mode_state &= FP_EXIT;
			ts_info("->FP_ENTER [%d]", FP_ENTER);
			ts_info("->FP_EXIT [%d]", FP_EXIT);
			break;
		case FP_GESTURE_ON:
			ts_core_data->tp_mode_state |= FP_ENTER;
			ts_info("->FP_ENTER [%d]", FP_ENTER);
			ts_info("->FP_EXIT [%d]", FP_EXIT);
			break;
		case NORMAL_GESTURE:
		...
		if(ts_core_data->bUpdateOver){
		msleep(10000);
		ts_info("success bUpdateOverk");
			}
	synaptics_nubiaCtl_Re_Sus(ts_core_data);
...
{
	if (!proc_mkdir("nubia_goodix_ts", NULL))
		return;
	proc_create("nubia_goodix_ts/tp_capacitance_data",
		S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, NULL,
		&rawdata_proc_fops);
	ts_err("goodix_ts_procfs_init, input null ptr");
		...
		struct goodix_touch_data *touch_data)
{
	...
	if (touch_num == 0)
		input_report_key(dev, BTN_TOUCH, 0);
#ifdef CONFIG_TOUCHSCREEN_GOODIX_TS_GESTURE
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data)
{
	...
	if (r < 0) {
		...
		ts_info("GPIO setup,avdd_gpio:%d, iovdd_gpio:%d",
		ts_bdata->avdd_gpio, ts_bdata->iovdd_gpio);
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	ts_info("goodix_ts_suspend, core_data->init_stage state %d", core_data->init_stage);
    return 0;
    }
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	if(core_data->init_stage < CORE_INIT_STAGE2 ||  
		  !atomic_read(&core_data->suspended)){
	ts_info("goodix_ts_resume, core_data->init_stage state %d", core_data->init_stage);
    return 0;
    }
...
out:
	...
	ts_info("Resume start disable_irq_wake");
	}
	ts_core_data->goodix_suspend = false;
	ts_info("ts_core_data->goodix_suspend:%x", ts_core_data->goodix_suspend);
	ts_debug("Resume end");
	return 0;
}
...
static void synaptics_nubiaCtl_Re_Sus(struct goodix_ts_core *core_data)
{
	int current_state;
	int message_source;
	current_state = ts_core_data->tp_mode_state;
	message_source = ts_core_data->fp_switch;
	switch(message_source) {
		case FP_MESSAGE:
			if ((current_state&FP_ENTER)&&ts_core_data->en_fpmode) {
				ts_core_data->en_fpmode = false;
				ts_info("->fp_resume  [%d]",current_state);
			    goodix_ts_suspend(core_data);				
			} else if(!(current_state&FP_ENTER)&&!(ts_core_data->en_fpmode)){
				ts_core_data->en_fpmode = true;
				ts_info("->fp_gesture [%d]", current_state);
		...
		case NORMAL_MESSAGE:
			...
			//		goodix_ts_resume(core_data);
				...
				case AOD_ON_FP_EXIT:
					ts_info(" -> suspend_%s [%d]",
					(core_data->goodix_suspend)?"gesture":"sleep", current_state);
					goodix_ts_suspend(core_data);
					break;
				default: break;
			}
	...
	unsigned long event, void *data)
{
	...
	if (!mndata || (mndata->id != 0))
		return 0;
    ts_core_data = core_data;
	if (mndata && mndata->data && core_data) {
		if ((event == MSM_DRM_EARLY_EVENT_BLANK)||(event == MSM_DRM_AOD_EVENT)){
			/* before fb blank */
			int *blank = mndata->data;
			switch(*blank) {
				case MSM_DRM_BLANK_POWERDOWN:
					core_data->tp_mode_state &= LCD_OFF;
					core_data->tp_mode_state &= AOD_OFF;
					break;
				case MSM_DRM_BLANK_UNBLANK:
					core_data->tp_mode_state |= LCD_ON;
					break;
				case MSM_DRM_MAJOR_AOD_ON:
					core_data->tp_mode_state |= AOD_ON;
					break;
			...
			synaptics_nubiaCtl_Re_Sus(core_data);
		}
	}
...
#ifdef CONFIG_FB
	cd->ts_notifier.notifier_call = goodix_ts_fb_notifier_callback;
	#ifdef CONFIG_TOUCHSCREEN_GOODIX_TS_GESTURE
	if (msm_drm_panel_register_client(&cd->ts_notifier))
...
static int goodix_later_init_thread(void *data)
{
	...
	ts_core_data->bUpdateOver = false;
	ts_info("ts_core_data->bUpdateOver:%x", ts_core_data->bUpdateOver);	
	return 0;
...
static int goodix_start_later_init(struct goodix_ts_core *ts_core)
{
	...
	ret1 = goodix_ts_stage2_init(ts_core);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	*/
	...
	core_data->ts_notifier.notifier_call = goodix_generic_noti_callback;
	ret = msm_drm_panel_register_client(&core_data->ts_notifier);
	if (ret < 0) {
		dev_err(&pdev->dev,
    ...
    */
    ts_core_data->bUpdateOver = true;
	ts_info("ts_core_data->bUpdateOver:%x", ts_core_data->bUpdateOver);

    ts_core_data->ts_notifier.notifier_call = goodix_ts_fb_notifier_callback;
#ifdef CONFIG_NUBIA_DISP_PREFERENCE
#ifdef CONFIG_TOUCHSCREEN_GOODIX_TS_GESTURE
	if (msm_drm_panel_register_client(&ts_core_data->ts_notifier))
		ts_err("Failed to register fb notifier client:%d", ret);
#endif
#endif
	...
	ts_core_data->goodix_suspend = false;
	ts_core_data->en_fpmode = false;
	ts_core_data->fp_switch = false;
	ts_core_data->tp_mode_state |= LCD_ON;
	ts_core_data->tp_mode_state &= AOD_OFF;
	ts_core_data->tp_mode_state &= FP_EXIT;
	...
	INIT_WORK(&ts_core_data->resume_work, goodix_resume_work_func);
