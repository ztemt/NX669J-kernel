...
*/
	...
	for (ii = 0; ii < config_id_size; ii++) {
		pr_info("synaptics:ui_cfg[%d]=%2x,dev_cfg[%d]=%2x\n",
				ii, fwu->img.ui_config.data[ii],
				ii, fwu->config_id[ii]);
		/* modify frimware for 6.65' TP '*/
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "firmware_id:%u\nconfig_id:%02x%02x%02x%02x\n",
		fwu->rmi4_data->firmware_id, fwu->config_id[28], fwu->config_id[29],
		fwu->config_id[30], fwu->config_id[31]);
}
#endif
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	pr_info("synaptics %s: on of fwu_scan_pdt\n", __func__);
	retval = fwu_scan_pdt();
	if (retval < 0)
		goto exit_free_mem;
