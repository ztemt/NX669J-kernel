...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	//wzm
   	prop1 = of_find_property(np, "synaptics,power1-gpio", NULL);
	if (prop1 && prop1->length) {
		bdata->power1_gpio = of_get_named_gpio_flags(np,
				"synaptics,power1-gpio", 0, NULL);
		retval = of_property_read_u32(np, "synaptics,power1-on-state",
				&value);
		if (retval < 0) {
			...
			bdata->power1_on_state = value;
		}
	} else {
		bdata->power1_gpio = -1;
	}
	pr_err("[TP]%s: synaptics,power1-on-state = %d \n", __func__, bdata->power1_on_state);
	//wzm
	...
	retval = of_property_read_string(np, "synaptics,firmware-image-name", &name);
	pr_info("%s: name=%s\n", __func__, name);
	if (retval < 0) {
		bdata->fw_image_name= NULL;
		pr_err("%s get fw_image_name failed:%d\n", __func__, retval);
	} else
		bdata->fw_image_name = name;
#endif
	return 0;
}
#endif
		...
		unsigned short addr)
{
	...
	if (page != rmi4_data->current_page) {
		...
		if(retry >= SYN_I2C_RETRY_TIMES) {
			pr_err("%s:--------CHECK ONE----------", __func__);
			pr_err("%s:i2c error ,check source,check state", __func__);
			pr_err("%s:current state %s", __func__, rmi4_data->suspend?"suspend":"resume");
		...
		const struct i2c_device_id *dev_id)
{
	...
	hw_if.board_data->client = client;
