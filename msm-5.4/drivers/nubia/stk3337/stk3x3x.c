...
#if ((STK_INT_PS_MODE == 0x03) || (STK_INT_PS_MODE  == 0x02))
        err = request_threaded_irq(stk_data->irq, NULL, stk_oss_irq_handler, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, DEVICE_NAME, stk_data);

#else
        err = request_threaded_irq(stk_data->irq, NULL, stk_oss_irq_handler, IRQF_TRIGGER_LOW | IRQF_ONESHOT, DEVICE_NAME, stk_data);
#endif
	if (err < 0) {
		SENSOR_LOG_DEBUG("request_threaded_irq(%d) failed for (%d)\n", irq, err);
