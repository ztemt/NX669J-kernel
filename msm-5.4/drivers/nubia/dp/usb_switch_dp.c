...
static int nubia_parse_switcher_dt(struct device_node *node)
{
	int rc = 0;
	u32 value;
	printk("entry nubia_parse_switcher_dt\n");
...
void get_dp_state(int *statue)
{
	*statue	= pswitcher->is_enabled;
}
EXPORT_SYMBOL(get_dp_state);
...
static int  nubia_usb_switch_dp_probe(struct platform_device *pdev)
{
	...
	printk("entry nubia_usb_switch_dp_probe\n");
	if (!pdev){
	...
	printk("end nubia_usb_switch_dp_probe\n");
	return 0;
