...
static ssize_t nubia_hw_pcb_ext_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    char pcb_ext[3][12]={"NX669J_V1B","NULL","NX669J_V1A"};
	...
	nubia_hw_version_debug("pcb_ver_ext=%s\n",buf);
	return sprintf(buf,"%s",buf);
}
static struct kobj_attribute pcb_version_ext_attr=
	__ATTR(pcb_version_ext, 0664, nubia_hw_pcb_ext_show, NULL);


static ssize_t nubia_hw_pcb_trans_show(struct kobject *kobj,
                struct kobj_attribute *attr, char *buf)
{
    char pcb_trans[3][12]={"NULL","NX669J","NX669J_V1E"};
