...
static int aw22xxx_load_nubia_fw_name(struct aw22xxx *aw22xxx)
{
    ...
    AW22_LOG("nubia_ver_gpio:%d, gpio_value:%d, fw name :%s", aw22xxx->nubia_ver_gpio, gpio_value, aw22xxx_fw_name);
    return 0;
}
...
{
	int i = 0;
	char buf[AW22XXX_CFG_NAME_MAX] = {0};

	for(i=0; i<AW22XXX_CFG_NUM_MAX; i++)
	{
		if(i == 0){
			strcpy(buf, "off.bin");
		}else if(i==3){			//just for factoty test
			strcpy(buf, "factory_test.bin");
		}else if(i>=0x50 && i<=0xaf){
			sprintf(buf, "aw_cfg_%x.bin", i + 0x50);
		}else{
			strcpy(buf, "null");
		}
		strcpy(aw22xxx_cfg_name[i], buf);
	}
}
/******************************************************
 *
...
#endif
#ifndef CONFIG_NUBIA_LED_AW22XXX_CTS
