...
#define AW8697_HAPTIC_NAME "aw8695_haptic"
...
#ifdef NUBIA_TOUCH_KEY
#define AW8697_PINCTRL_STATE_ACTIVE "aw8697_int_active"
#define AW8697_PINCTRL_STATE_SUSPEND "aw8697_int_suspend"
#endif

#define VIBRATOR_CAL_FILE_PATH        "/persist/sensors/vibrator_cal_data"
int calibiration_flag = 0;
...
static char aw8697_rtp_name[][AW8697_RTP_NAME_MAX] = {
	...
	{"aw8697_rtp_silk.bin"},
	{"1-fastblood_RTP.bin"},
	{"22-youhaveslainanenemy_RTP.bin"},
	{"3-doulekill_RTP.bin"},
	{"4-triplekill_RTP.bin"},
	{"5-quadrakill_RTP.bin"},
	{"6-pentakill_RTP.bin"},
	{"7-hexa6_RTP.bin"},
	{"8-hepta7_RTP.bin"},
	{"9-octa8_RTP.bin"},
	...
	{"12-c5_RTP.bin"},
	{"13-c6_RTP.bin"},
	{"14-engendary_RTP.bin"},
	{"18-victory_RTP.bin"},
	{"19-defeat_RTP.bin"},
	{"23-da_RTP.bin"},
	{"24-xiao_RTP.bin"},
	{"21-pisuccess_RTP.bin"},
};
...
/*****************************************************
 *
 * vibrator_write_file vibrator_read_file
 *
 *****************************************************/

int vibrator_read_file(char *file_path, char *read_buf, int count)
{
#ifdef CONFIG_NUBIG_HAPTIC_GKI
	return 0;
#else
        struct file *file_p;
        mm_segment_t old_fs;
...
static void aw8697_haptic_audio_work_routine(struct work_struct *work)
{
	...
	if (aw8697->haptic_audio.ctr.cmd == AW8697_HAPTIC_CMD_ENABLE) {
		if (aw8697->haptic_audio.ctr.play == AW8697_HAPTIC_PLAY_ENABLE) {
			...
			do_gettimeofday(&aw8697->audio_trig);
			mutex_lock(&aw8697->lock);
			aw8697->touch_audio_gap = (aw8697->audio_trig.tv_sec - aw8697->touch_press.tv_sec)*1000000 +
				(aw8697->audio_trig.tv_usec - aw8697->touch_press.tv_usec);
			pr_info("%s: haptic_audio_play_start touch_audio_gap %ld \n", __func__,aw8697->touch_audio_gap);
			if (((aw8697->touch_audio_gap >= aw8697->gap_min) && (aw8697->touch >= 1) && (aw8697->touch_enable == AW8697_HAPTIC_TOUCH_ENABLE))
					|| ((aw8697->touch_audio_gap <= aw8697->gap_max) && (aw8697->touch < 1) && (aw8697->touch_enable == AW8697_HAPTIC_TOUCH_ENABLE))
					||(aw8697->touch_enable == AW8697_HAPTIC_TOUCH_DISABLE))
			{
				//vincent surport touch
				pr_info("%s: haptic_audio_play_start real play\n", __func__);
				aw8697_haptic_stop(aw8697);
				aw8697_haptic_play_mode(aw8697, AW8697_HAPTIC_RAM_MODE);
				...
				if (aw8697->haptic_audio.ctr.loop == 15 )
				{
					aw8697->haptic_audio.ctr.loop = 14;
				}
				//end modify huazhonghua zhixuzhendong
...
static int aw8697_haptic_f0_calibration(struct aw8697 *aw8697)
{
	...
	if (aw8697_haptic_get_f0(aw8697)) {
		...
		printk("%s f0_cali_lra=%d\n", __func__, (int)f0_cali_lra);
		aw8697->vibrator_factory = (int)f0_cali_lra;

		vibrator_write_file(VIBRATOR_CAL_FILE_PATH,(char *)(&aw8697->vibrator_factory), sizeof(aw8697->vibrator_factory));
		ret = vibrator_read_file(VIBRATOR_CAL_FILE_PATH, (char *)(&aw8697->vib_factor),sizeof(aw8697->vib_factor));
		pr_info("%s yangxianchang =%d  ret = %d\n", __func__, aw8697->vib_factor,ret);
		/* update cali step */
...
static int aw8697_haptic_init(struct aw8697 *aw8697)
{
	...
	aw8697->gap_min = 10000;
	aw8697->gap_max = 500000;
...
static ssize_t aw8697_touch_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8697_touch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aw8697->touch = val;
	do_gettimeofday(&aw8697->touch_press);
...
static ssize_t aw8697_touch_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8697_touch_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8697_gap_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	/* For now nothing to show */
	return snprintf(buf, PAGE_SIZE, "gap_min: %d, gap_max: %d\n", aw8697->gap_min, aw8697->gap_max);
}

static ssize_t aw8697_gap_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	if(2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
			pr_debug("%s: gap_min: 0x%x, gap_max: 0x%x\n", __FUNCTION__, databuf[0], databuf[1]);
			mutex_lock(&aw8697->lock);
			aw8697->gap_min = databuf[0];
			aw8697->gap_max = databuf[1];
			mutex_unlock(&aw8697->lock);
	}
				  ...
				  const char *buf, size_t count)
{
	...
	pr_debug("%s: value=%d\n", __func__, val);
	err = aw8697_load_vibrator_calibiration(aw8697);
	pr_debug("%s: 11111valu erere=%d\n", __func__, err);
	mutex_lock(&aw8697->lock);
	aw8697->index = val;
...
static int32_t aw8697_load_vibrator_calibiration(struct aw8697 *aw8697)
{
	int ret = 0;
	ret = vibrator_read_file(VIBRATOR_CAL_FILE_PATH, (char *)(&aw8697->vib_factor),sizeof(aw8697->vib_factor));

	pr_debug("%s: aw8697_load_vibrator_calibiration value=%d\n", __func__, (aw8697->f0_calib_data));

	if (ret < 0 )
	    return -1;
	aw8697->f0_calib_data = aw8697->vib_factor;
	pr_err("%s: 33333value=%d\n", __func__, (aw8697->f0_calib_data));
	return 0;
}
...
static DEVICE_ATTR(touch, S_IWUSR | S_IRUGO, aw8697_touch_show, aw8697_touch_store);
static DEVICE_ATTR(touch_enable, S_IWUSR | S_IRUGO, aw8697_touch_enable_show, aw8697_touch_enable_store);
//vincent surport touch
...
static DEVICE_ATTR(gap, S_IWUSR | S_IRUGO, aw8697_gap_show, aw8697_gap_store);
...
static void aw8697_vibrator_work_routine(struct work_struct *work)
{
	...
	pr_debug("%s enter\n", __func__);
	mutex_lock(&aw8697->lock);
	if(calibiration_flag == 0){
	    vibrator_read_file(VIBRATOR_CAL_FILE_PATH, (char *)(&aw8697->vib_factor),sizeof(aw8697->vib_factor));
	    pr_info("%s aw8697_vibrator_work_routine aw8697->vib_factor  =%d\n", __func__, aw8697->vib_factor);
	    cali_cal_data = aw8697->vib_factor;
	    pr_info("%s aw8697_vibrator_work_routine aw8697->f0_calib_data = %d \n", __func__, aw8697->f0_calib_data);
	    aw8697_i2c_write(aw8697, AW8697_REG_TRIM_LRA, (char)cali_cal_data);
	    aw8697_i2c_read(aw8697, AW8697_REG_TRIM_LRA, &reg_val);
		
	}
	calibiration_flag = 1;
	pr_info("%s aw8697_vibrator_work_routine not calibiration  = %d \n", __func__, aw8697->f0_calib_data);
	aw8697_haptic_stop(aw8697);
...
#ifdef NUBIA_TOUCH_KEY
static int aw8697_pinctrl_init(struct device *dev, struct aw8697 *aw8697_data)
{
	pr_info("%s enter\n", __func__);
	aw8697_data->pinctrl_info.pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(aw8697_data->pinctrl_info.pinctrl)) {
		pr_err("%s: aw8697 get pinctrl info error.\n", __func__);
		return -EINVAL;
	}

	aw8697_data->pinctrl_info.pin_active = pinctrl_lookup_state(
			aw8697_data->pinctrl_info.pinctrl,
			AW8697_PINCTRL_STATE_ACTIVE);
	if (IS_ERR_OR_NULL(aw8697_data->pinctrl_info.pin_active)) {
		pr_err("%s: aw8697 get pin_active info error.\n",__func__);
		return -EINVAL;
	}

	aw8697_data->pinctrl_info.pin_suspend = pinctrl_lookup_state(
			aw8697_data->pinctrl_info.pinctrl,
			AW8697_PINCTRL_STATE_SUSPEND);
	if (IS_ERR_OR_NULL(aw8697_data->pinctrl_info.pin_suspend)) {
		pr_err("%s: aw8697 get pin_suspend info error.\n",__func__);
		return -EINVAL;
	}
	pr_info("%s exit\n", __func__);
	return 0;
...
static int aw8697_pinctrl_set_state( struct aw8697 *aw8697_data,  bool active )
{
	int ret = -1;
	pr_info("%s enter\n", __func__);
	if (!aw8697_data->pinctrl_info.pinctrl ||
			!aw8697_data->pinctrl_info.pin_active ||
			!aw8697_data->pinctrl_info.pin_suspend) {
		pr_err("%s: pinctrl is invalid, skip.\n",__func__);
		return ret;
	}
	if (active) {
		ret = pinctrl_select_state(aw8697_data->pinctrl_info.pinctrl,
				aw8697_data->pinctrl_info.pin_active);
	} else {
		ret = pinctrl_select_state(aw8697_data->pinctrl_info.pinctrl,
				aw8697_data->pinctrl_info.pin_suspend);
	}
	pr_info("%s: set pinctrl to [%s], ret = %d.\n", __func__,
			    ...
			    const struct i2c_device_id *id)
{
...
#ifdef NUBIA_TOUCH_KEY
	ret = aw8697_pinctrl_init(&i2c->dev, aw8697);
	if (ret < 0) {
		pr_err("%s: aw8697 pinctrl init failed.\n", __func__);
		goto pinctrl_init_failed;
	}
	ret = aw8697_pinctrl_set_state(aw8697, true);
	if (ret) {
		pr_err("%s: aw8697 pinctrl set state failed.\n", __func__);
		goto pinctrl_set_failed;
	}
	pr_debug("%s: aw8697 pinctrl inited, set state to active successfully.\n", __func__);
#endif
	...
	vibrator_read_file(VIBRATOR_CAL_FILE_PATH, (char *)(&aw8697->vib_factor),sizeof(aw8697->vib_factor));
	pr_info("%s yangxianchang probe  =%d\n", __func__, aw8697->vib_factor);
	/* aw8697 irq */
	if (gpio_is_valid(aw8697->irq_gpio) &&
	    !(aw8697->flags & AW8697_FLAG_SKIP_INTERRUPTS)) {
...
#ifdef NUBIA_TOUCH_KEY
pinctrl_set_failed:
	devm_pinctrl_put(aw8697->pinctrl_info.pinctrl);
pinctrl_init_failed:
	aw8697->pinctrl_info.pinctrl = NULL;
#endif
#ifndef CONFIG_NUBIG_HAPTIC_GKI
	if (gpio_is_valid(aw8697->irq_gpio))
