...
enum blink_mode {
	AW_CONST_OFF = 0, // 0 darkness is comming
	AW_CONST_ON, // 1 work on a constant lightness.
	AW_AUTO_BREATH, // 2 self breathing, used in sences such as missing message.
	AW_STEP_FADE_IN, // 3  fade in means that the lightness is getting stronger.
	AW_STEP_FADE_OUT, // 4  fade out means that the lightness is getting weaker
	AW_BREATH_ONCE, // 5 only breath once, touch the home menu for instance.
};
typedef struct {
	u32 breath_mode;
	u32 fade_time;
