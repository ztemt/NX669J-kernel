...
static void sec_nfc_set_mode(struct sec_nfc_info *info, enum sec_nfc_mode mode);
...
void sec_cardsimulation_detect(char direct, u8 *data, int len)
{
	static char start = 0;
	...
	if(data[0]==0x61 && data[1]==0x05){
		printk(KERN_ERR "NB_NFC:chip->ap;Notification Interface actived!");
		start = 1;
	}
	if(data[0]==0x61 && data[1]==0x06){
		printk(KERN_ERR "NB_NFC:chip->ap;Notification Deactivate!");
		if(start){
			printk(KERN_ERR "NB_NFC:Detect a successful cardsimulation!");
		}else{
			printk(KERN_ERR "NB_NFC:Lost Notification actived, maybe it is a bad cardsimulation!");
		}
		start = 0;
	}
}
							...
							size_t count, loff_t *ppos)
{
	...
	sec_cardsimulation_detect(1, info->i2c_info.buf, ret);
...
detect_end:
    sec_nfc_set_mode(info, SEC_NFC_MODE_OFF);
    if(ret){
        printk(KERN_DEBUG "%s:SEC NFC chip detect FAILED!.\n", __func__);
    }else{
        printk(KERN_DEBUG "%s:SEC NFC chip detect Ok! chip_type=%02x.\n", __func__, rx_buff[4]);
    }
...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
	...
	ret = sec_nfc_chip_detect(info);
        if(ret){
            pr_err("sec_nfc_chip_detect failed.ret=%d\n",ret);
	    return ret;
        }
        pr_err("sec_nfc_chip_detect success.\n");
...
int trig_cold_reset_id(int id)
{
	...
	printk(KERN_ERR "NB_NFC:%s: enter.", __func__);
	...
	printk(KERN_ERR "NB_NFC:%s: success.", __func__);
	return 0;
}

int trig_cold_reset(void) // Called from eSE driver
{
...
{
	pr_info("%s trig_nfc_wakeup cur_mode=%d.\n", __func__, cur_mode);
...
{
	pr_info("%s trig_nfc_sleep cur_mode=%d.\n", __func__, cur_mode);
	if (cur_mode == SEC_NFC_MODE_OFF || cur_mode == SEC_NFC_MODE_BOOTLOADER) {
...
int trig_cold_reset(void) // Called from eSE driver
{
	pr_info("%s trig_cold_reset is null.\n", __func__);
...
int trig_nfc_wakeup(void)
{
    pr_info("%s trig_nfc_wakeup is null.\n", __func__);
	return 0;
}
EXPORT_SYMBOL(trig_nfc_wakeup);
...
int trig_nfc_sleep(void)
{
    pr_info("%s trig_nfc_sleep is null.\n", __func__);
	return 0;
}
EXPORT_SYMBOL(trig_nfc_sleep);
#endif
...
static int sec_pinctrl_active(struct device *dev)
{
    ...
    err = pinctrl_select_state(pinctrl, gpio_pins_active);
        ...
        pr_info("%s: sec-nfc pincontrol set success!\n", __func__);
