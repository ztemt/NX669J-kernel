...
static ssize_t fan_speed_level_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
...
static void fan_monitor(void){
	...
	if((get_fan_power_on_state() == true) ){
		...
		if(fan_count<=FAN_MIN_RST_COUNT || fan_count>=FAN_MAX_RST_COUNT){	//speed error
			err_cnt++;
			ck_delay = 200;
		}else{
			err_cnt = 0;
...
static ssize_t fan_pwm_store(struct kobject *kobj,
	    struct kobj_attribute *attr, const char *buf, size_t count){
