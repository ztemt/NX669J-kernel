...
static int dp_altmode_notify(void *priv, void *data, size_t len)
{
	...
	/* touping connect before usb host connect then return and wait the usb connect first */
	get_usb_state(&state);
	NUBIA_USB_INFO("altmode->connected = %d, usb_host_state = %d.\n", altmode->connected, state);
	if (!altmode->connected && !state)
	{
		goto ack;
	}
