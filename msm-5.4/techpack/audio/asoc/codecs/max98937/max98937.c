...
extern int afe_dsm_set_adsp_mode(uint8_t* payload);
...
static struct max989xx_priv *max98927_debug = NULL;
...
int max98937_reg_common_map[][2] = {
    {MAX98937_Clock_monitor_enable, 0x00},
...
//Speaker DC resistance is 7.3 +/-10% (6.57 --> 8.03)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
//#define SPEAKER_RDC_MIN  (264807949)  // 6.57 / 3.33 * (1<<27)
//#define SPEAKER_RDC_MAX  (323654161)  // 8.03 / 3.33 * (1<<27)
//#define SPEAKER_RDC_DEFAULT (294231055)  // 7.3 / 3.33 * 134217728

//Receiver DC resistance is 7.5 +/-10% (6.75 --> 8.25)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
...
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
//#define SPEAKER_RDC_MIN  (241833744)  // 6.00 / 3.33 * (1<<27)
//#define SPEAKER_RDC_MAX  (314383867)  // 7.80 / 3.33 * (1<<27)
//#define SPEAKER_RDC_DEFAULT (278108805)  // 6.90 / 3.33 * 134217728

//Receiver DC resistance is 7.1 +/-13% (6.10 --> 8.00)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
...
//Speaker DC resistance is 6.8 +/-10% (6.12 --> 7.48)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
#define SPEAKER_RDC_MIN  (246670419)  // 6.12 / 3.33 * (1<<27)
#define SPEAKER_RDC_MAX  (301486068)  // 7.48 / 3.33 * (1<<27)
#define SPEAKER_RDC_DEFAULT (274078243)  // 6.80 / 3.33 * 134217728

//Receiver DC resistance is 6.3 +/-10% (5.67 --> 6.93)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
#define EAR_RDC_MIN  (228532888)  // 5.67 / 3.33 * (1<<27)
#define EAR_RDC_MAX  (279317975)  // 6.93 / 3.33 * (1<<27)
#define EAR_RDC_DEFAULT (253925431)  // 6.30 / 3.33 * 134217728
...
#define CALIBRATE_FILE_L_APP   "/data/spkr_calib_l.txt"
#define CALIBRATE_FILE_R_APP   "/data/spkr_calib_r.txt"
...
static int max989xx_calib_get(uint32_t* calib_value, int ch)
{
	...
	*calib_value = 0;
	...
	if (!IS_ERR_OR_NULL(pfile)) {
		...
		__kernel_read(pfile, (char *)calib_value, sizeof(uint32_t), &pos);   
	...
	found = 1;
	if (ch == MAX98927L)
		*calib_value = EAR_RDC_DEFAULT;
	else
		*calib_value = SPEAKER_RDC_DEFAULT;
#endif
	return found;
}
...
static int max989xx_calib_save (uint32_t calib_value, int ch)
{
	...
	int bytes_write =0;
	const char * filename = NULL;
	const char * filename_app = NULL;

	if (ch == MAX98927L) {
		filename = CALIBRATE_FILE_L;
		filename_app = CALIBRATE_FILE_L_APP;
	}
	else if (ch == MAX98927R) {
		filename = CALIBRATE_FILE_R;
		filename_app = CALIBRATE_FILE_R_APP;
	}
	else {
		pr_err("%s: invalid ch: %d\n", __func__, ch);
		return -1;
	...
	pfile_app = filp_open(filename_app, O_RDWR | O_CREAT, 0666);
	if (!IS_ERR(pfile) || !IS_ERR(pfile_app)) {
		pr_info("%s: save2 %s, calib_value=%d\n",__func__, filename, calib_value);
		//vfs_write(pfile, (char *)&calib_value, sizeof(uint32_t), &pos);
		__kernel_write(pfile, (char *)&calib_value, sizeof(uint32_t), &pos);
		bytes_write = sprintf(buf, "%d", calib_value);
		//vfs_write(pfile_app, buf, bytes_write, &pos_app);
		__kernel_write(pfile_app, buf, bytes_write, &pos_app);
		filp_close(pfile, NULL);
		filp_close(pfile_app, NULL);
	} else {
...
#if 1		 
static ssize_t max989xx_dbgfs_calibrate_write(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct max989xx_priv *max98927 = max98927_debug;
...
static struct kobj_attribute max989xx_debug_attrs[] = {
	__ATTR(calibrate, 0664, NULL, max989xx_dbgfs_calibrate_write),
};

static int __init max989xx_debug_init(void)
{
	int retval = 0;
	int attr_count = 0;
	...
	enhance_kobj = kobject_create_and_add("max98927L", kernel_kobj);

	if (!enhance_kobj) {
		pr_err("%s: enhance_kobj is null \n", __func__);
		return -ENOMEM;
	}

	/* Create attribute files associated with this kobject */
	for (attr_count = 0; attr_count < ARRAY_SIZE(max989xx_debug_attrs); attr_count++) {
		retval = sysfs_create_file(enhance_kobj, &max989xx_debug_attrs[attr_count].attr);
		if (retval < 0) {
...
err_sys_creat:
	for (--attr_count; attr_count >= 0; attr_count--)
		sysfs_remove_file(enhance_kobj, &max989xx_debug_attrs[attr_count].attr);

	kobject_put(enhance_kobj);
	return retval;
}

static void __exit max989xx_debug_remove(void)
{
	int attr_count = 0;
...
static ssize_t max989xx_dbgfs_calibrate_write(struct file *file,
...
static const struct file_operations max989xx_dbgfs_calibrate_fops = {
	...
	.write = max989xx_dbgfs_calibrate_write,
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(codec);
...
static int max98927_stream_mute(struct snd_soc_dai *codec_dai, int mute, int stream)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
            max98927_wrapper_read(max98927, 0, MAX98937_Meas_ADC_Chan_0_Readback, &(max98927->level_hold));
            if (max98927->level_hold == 0) {
                max98927_wrapper_read(max98927, 1, MAX98937_Meas_ADC_Chan_0_Readback, &(max98927->level_hold));
            }
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	*(payload+2) = max98927->adsp_mode;
	//afe_dsm_set_calib((uint8_t *)payload);
	afe_dsm_set_adsp_mode((uint8_t *)payload);
	//load calibration to DSM
	mutex_unlock(&dsm_lock);
			      ...
			      const struct i2c_device_id *id)
{
	...
	//max98927->regmap[id->driver_data] =devm_regmap_init_i2c(i2c, &max989xx_initial_regmap);
	max98927->regmap[id->driver_data] =devm_regmap_init_i2c(i2c, &max98937_regmap);
